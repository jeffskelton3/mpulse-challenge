...
def __get_upsert_member_ids(member_account_list: [MemberAccountBody]):
    client_member_ids = list(map(lambda item: item["client_member_id"], member_account_list))
    select_statement: Select = sa.select([Member.member_id, Member.client_member_id],
                                         Member.client_member_id.in_(client_member_ids))
    parsed_results = {}
    with get_session() as session:
        result: Result = session.execute(select_statement)
        for row in result.all():
            parsed_results[row[1]] = {
                "member_id": row[0]
            }
